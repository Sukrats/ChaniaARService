...
public class UserService {
	...
	public static ArrayList<Profile> getAllProfiles(){
        ...
        try{
	        ...
	        while (rs.next()) {
	        	...
	        	profile.setId(rs.getLong("player_id"));
	...
	public static Profile addProfile(Profile profile){
		...
		try {
            ...
            String query = "INSERT into player( username, email, firstname, lastname, password, score, created, recent_activity) "
            		+ "values('"+profile.getUsername()+ "',"+"'"+ profile.getEmail()+
            		"','"+profile.getFirstname() + "','"+profile.getLastname() +
            		"','"+profile.getPassword()+ "','"+profile.getScore() +
            		"', CURDATE(), UTC_TIMESTAMP() )";
            System.out.println(query);
	...
	public static ArrayList<Place> getPlaces(String username){
		...
		try {
			...
			/*String query = "SELECT p.scene_id , pl.username , p.comment , p.created "+
			"FROM (places AS p JOIN player AS pl ON p.player_id = pl.player_id)" +
			"WHERE pl.username = '"+ username +"';";*/

			String query = "SELECT "
					+ " p.scene_id , pl.username , p.comment , p.created , a.admin_area_name ,"
					+ " s.name , s.numofvisits , s.numofsaves "
					+ " FROM ((places AS p JOIN player AS pl ON p.player_id = pl.player_id)"
					+ " INNER JOIN scene AS s ON s.scene_id = p.scene_id ) "
					+ " INNER JOIN levels AS a ON s.admin_area_id = a.admin_area_id"
					+ " WHERE pl.username = '"+ username +"' AND s.scene_id = p.scene_id;";
			
			ResultSet rs = stmt.executeQuery(query);
			while(rs.next()){
				Place p = new Place();
				p.setScene_id(rs.getLong("scene_id"));
				p.setComment(rs.getString("comment"));
				p.setUsername(rs.getString("username"));
				p.setAdmin_area_name(rs.getString("admin_area_name"));
				p.setScene_name(rs.getString("name"));
				p.setScene_visits(rs.getString("numofvisits"));
				p.setScene_saves(rs.getString("numofsaves"));
				places.add(p);
			}
	...
	public static Place getPlace(String username, long sceneid){
		...
		try {
			...
			String query = "SELECT p.scene_id , pl.username , p.comment , p.created "+
			"FROM (places AS p JOIN player AS pl ON p.player_id = pl.player_id)"+
			"WHERE pl.username = '"+ username +"' AND scene_id = '"+sceneid + "';";
			
			ResultSet rs = stmt.executeQuery(query);
			if(rs.next()){
				p= new Place();
				p.setScene_id(rs.getLong(1));
	...
	public static boolean addPlace(String username, long sceneid){
		...
		try {
			...
			String query = "INSERT INTO places (player_id, scene_id, comment, created) VALUES("+profile.getId()+","+
			"'"+sceneid+"','"+"', CURDATE()"+");";
	...
	public static boolean updatePlace( Place place){
		...
		try {
			conn = DBUtility.createConnection();
			stmt = conn.createStatement();
			Profile profile = getProfile(place.getUsername());
			String update = "UPDATE places SET places.comment = '" + place.getComment()+"', places.created = CURDATE()"
					+ "WHERE player_id = "+profile.getId()+" AND scene_id = '"+place.getScene_id()+"';";
	...
	public static boolean deletePlace(String username, long sceneid){
		...
		try {
			...
			Profile profile = getProfile(username);
			String delete = "DELETE FROM places WHERE places.player_id = "+profile.getId()+" AND scene_id = '"+ sceneid+"';";
	...
	public static ArrayList<Visit> getVisits(String username){
		...
		try {
			...
			+ " FROM (visits AS v JOIN player AS pl ON v.player_id = pl.player_id) "
			+ " JOIN scene AS s ON v.scene_id = s.scene_id "
			+ " WHERE pl.username = '"+ username +"';";*/

			String query = "SELECT "
					+ " v.scene_id , pl.username , v.created , a.admin_area_name ,"
					+ " s.name , s.numofvisits , s.numofsaves "
					+ " FROM ((visits AS v JOIN player AS pl ON v.player_id = pl.player_id)"
					+ " INNER JOIN scene AS s ON s.scene_id = v.scene_id ) "
					+ " INNER JOIN levels AS a ON s.admin_area_id = a.admin_area_id"
					+ " WHERE pl.username = '"+ username +"' AND s.scene_id = v.scene_id;";
			
			ResultSet rs = stmt.executeQuery(query);
			while(rs.next()){
				Visit v = new Visit();
				v.setScene_name(rs.getString("name"));
				v.setAdmin_area_name(rs.getString("admin_area_name"));
				v.setScene_visits(rs.getString("numofvisits"));
				v.setScene_saves(rs.getString("numofsaves"));
				v.setUsername(rs.getString("username"));
				v.setCreated(rs.getDate("created"));
				v.setScene_id(rs.getLong("scene_id"));
				visits.add(v);
			}
	...
	public static boolean addVisit(String username, long sceneid){
		...
		try {
			...
			stmt = conn.createStatement();
			Profile profile = getProfile(username);
			String query = "INSERT INTO visits ( scene_id, player_id, created) VALUES('"+sceneid+"',"+profile.getId()+","+
			" CURDATE() );";
