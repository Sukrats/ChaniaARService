...
public class ScenesResource {
	...
	@GET
	public List<Scene> getScenes(@Context UriInfo uriInfo,
			@QueryParam( "period")long periodId,
			@QueryParam( "lat" )double latitude,
			@QueryParam( "lon" )double longitude,
			@QueryParam( "country" )String country,
			@QueryParam( "area" )String admin_area){
		...
		if(periodId > 0){
		...
		else if(country != null && admin_area != null){
			System.out.print("Looking For Scenes in: "+country+admin_area);
			scenes = SceneService.getScenesForLocation(country, admin_area);
			for(Scene scene: scenes){
				scene.addLink(new Link(uriForSelf(uriInfo, scene),"self"));
				if(!scene.getImgLoc().isEmpty()){
					scene.addLink(new Link(uriForThumbnail(uriInfo, scene),"thumbnail"));
			...
			System.out.print("Looking For Scenes in: "+latitude+longitude);
			scenes = SceneService.getScenesForAbsoluteLocation(new Location(latitude, longitude));
			for(Scene scene: scenes){
				scene.addLink(new Link(uriForSelf(uriInfo, scene),"self"));
				if(!scene.getImgLoc().isEmpty()){
					scene.addLink(new Link(uriForThumbnail(uriInfo, scene),"thumbnail"));
